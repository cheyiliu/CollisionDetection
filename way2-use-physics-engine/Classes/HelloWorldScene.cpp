...
//
			        if(spriteA && spriteA->getTag() == TAG_PLANE
			        		&& spriteB && spriteB->getTag() == TAG_BULLET ){
			        	planeBlowup(spriteA);
			        	removeChild(spriteB);
			        }else if(spriteA && spriteA->getTag() == TAG_BULLET
			        		&& spriteB && spriteB->getTag() == TAG_PLANE ){
			        	planeBlowup(spriteB);
			        	removeChild(spriteA);
			        }
...
void HelloWorld::buildPlane(int startX, int endX, int actualY) {
	...
	auto sp = Sprite::createWithSpriteFrameName("hero1.png");
